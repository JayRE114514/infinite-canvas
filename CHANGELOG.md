...
+ [新增] 服务端新增 Workspace 精确整数积分账本、管理员幂等赠送和成员余额查询，账务分录不可变并在提交时强制零和与 Wallet 投影一致。
+ [新增] 服务端新增 Billing Order/Credit Hold、S3 Asset 生命周期、pg-boss 原子任务、租约代际 Worker 和所有者维护的 OpenAI-compatible/Gemini native 图片 Adapter，支持成功结算、明确失败释放与不明确结果对账保留。
+ [新增] 图片工作台新增平台积分异步生成入口，通过持久 Task Event/SSE 展示进度，并仅以稳定 assetId 从项目后端同源流式读取 ready 结果。
+ [修复] 平台图片任务统一规范化数据库事件时间，避免 SSE 反复断开并使任务终态与余额停留在旧状态。
+ [修复] 文档站默认英文路径不再因内部语言重写产生重定向循环。
+ [优化] 文档站移动端折叠菜单新增分类切换入口，桌面端增加随滚动高亮的本页目录。
+ [新增] 前端新增邮箱注册登录、受保护应用壳、独立标签页 Workspace 切换、成员邀请管理及邀请链接接受流程。
+ [修复] 邀请接受按账号与邀请长期去重、隔离路由状态并拆分可重试同步阶段，会话清理后旧请求不再回写 Workspace 状态。

+ [新增] 服务端新增个人与团队空间、成员和邀请接口，并按路径空间实时校验成员权限。
+ [新增] 服务端新增云端画布数据表与共享画布契约，画布 ID 使用数据库生成的 UUID，快照存 JSONB，并带 revision 版本号用于后续并发写入校验。
+ [新增] 服务端新增 Workspace 范围的云端画布增删改查接口，支持 revision 冲突保护、10 MiB 快照限制和重复幂等软删除。
+ [调整] 云端画布外键不再级联删除，注销普通成员账号只清空署名、物理删除空间被拒绝，画布快照仅接受合法 JSON 值（含换行、行分隔符等特殊键名下的值），revision 上下界同时由契约和数据库约束限制。
+ [修复] 服务端请求校验不再静默移除契约外字段，非法 JSON 与过大请求统一返回脱敏的稳定错误。
+ [修复] Workspace 创建、邀请接受与待处理邀请查询改由应用事务和权限接口负责，关闭公开 Organization 业务路由并强化生产 Cookie 与邀请并发约束。
+ [修复] 邀请接受在成员上限处按 Workspace 串行化，避免不同邀请并发接受时突破 100 人限制。

+ [调整] 画布列表、新建、打开、保存、重命名和删除全部改由服务端 Workspace 云端画布接口负责，前端不再本地持久化画布列表。
+ [新增] 画布编辑改为 400 毫秒防抖的云端保存，并按账号、Workspace、画布和基线版本写本地草稿，保存成功后清理旧草稿。
+ [新增] 画布出现版本冲突时停止该画布的自动保存，并提供「载入服务端版本」和「导出本地草稿」两个显式操作，不做任何自动合并或覆盖。
+ [调整] 切换账号或 Workspace 会立即清空内存中的画布列表、当前画布、保存计时器与冲突状态，旧作用域的迟到响应不再回写。
+ [调整] 本地画布不再自动上传，导入云端画布需要显式选择画布压缩包；WebDAV 同步不再包含画布域。
+ [修复] 媒体文件回收改为只清理明确释放的键，避免服务端画布仍在引用的图片和视频被误删。
+ [修复] 云端画布打开不再触发空保存，自动保存会跨导航保留捕获的快照并串行推进 revision，冲突、作用域切换、草稿存储失败及批量删除部分失败不再破坏后续编辑。
+ [修复] 云端画布按画布持久化冲突标记并隔离迟到加载，编辑立即写本地草稿，保存超时与本地存储/媒体补水故障不再永久阻塞后续使用。
+ [修复] 冲突后「载入服务端版本」改为取回与应用两段提交，服务端内容完全渲染前画布不可编辑，避免本地冲突内容被静默保存回服务端。
+ [优化] 画布本地草稿改为短窗口合并写入，连续拖动不再逐帧序列化整份快照，同一画布同时只保留最后一条待写草稿。
+ [新增] 本地草稿状态读取失败时暂停云端保存并给出可重试的提示，确认没有未知冲突后才恢复自动保存。
+ [新增] 画布顶部新增保存状态提示，保存中、已保存、保存失败与本地恢复失败都可见，失败可直接重试。
...
+ [调整] 画布本地草稿恢复改用独立的原生 IndexedDB 数据库，判定与写入在同一事务内按 CAS 提交：未同步草稿不会被其他标签页静默覆盖或仅按时间回收，只有拿到匹配的删除回执才会本地墓碑化，跨身份互不可见，删除结果也会区分「服务端已删除但本地清理待重试」和「未收到删除回执」。
...
+ [修复] 删除当前画布改为先冻结再按服务端结果裁决，删除失败时同一个会话与最后一次编辑原样恢复，只有确认删除的画布才清理本地记录。
+ [修复] 画布被替换时的收尾会等待并处理自己在飞的那次保存，必要时按返回的最新版本再补一次保存，成功后原地改写草稿，重新打开不再出现假冲突。
+ [修复] 本地恢复重试会完整重新判定草稿与冲突标记，存在未解决的待保存草稿时保持冲突并提升为标记，不再直接解锁自动保存。
+ [修复] Agent 生成结果按发起时的画布与就绪代际归属，切换画布后迟到的生成不再改写、提示或清理另一张画布的节点状态。
