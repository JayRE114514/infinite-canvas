...
状态：第五轮收敛审查问题已修订，待最终固定提交复审与用户复核；本文件通过前不授权继续实施
...
`canvases`：`id`、`workspace_id`、`title`、`document_mode`、`snapshot_json`、`revision`、创建/更新用户和时间、`deleted_at`、可空 `deletion_receipt_id`。`document_mode` 使用 `snapshot | collaborative` CHECK/枚举且默认 `snapshot`，第一阶段请求不得修改；`snapshot_json` 在 collaborative 模式中不得作为文档权威。`deletion_receipt_id` 在首次软删除时生成并与 `deleted_at` 同时提交，未删除时必须为空，幂等 DELETE 重放返回同一 ID。`revision` 在数据库使用 `BIGINT`，但传输与 TypeScript 明确限制在 `Number.MAX_SAFE_INTEGER`，这是版本号的特例，绝不能套用到积分金额。
...
- owner 自助 deactivation 使用独立、命令专用的 `TO app_api` UPDATE policy：`USING` 只接受当前用户拥有的 active team Workspace，`WITH CHECK` 只接受同一 owner、同一 team 类型的 deactivated + 非空 `deleted_at` 终态；personal Workspace 默认拒绝。普通成员更新 policy 仍只允许 active→active。路由 SQL 只能修改状态和删除时间，不能借该分支改 owner、类型或其他字段。
...
删除云端 Canvas 使用状态幂等的 DELETE：活动成员首次软删除或重放同一已删除 Canvas 时返回 `200`，包含匹配的 `canvasId`、`deletedAt` 和不可被普通查询伪造的 `deletionReceipt`；无权限、Workspace 非 active 或不可见资源仍返回普通拒绝/404，绝不返回删除回执。前端只有收到该 DELETE 正向回执才能写本地永久 tombstone。
...
- marker 变更、外部草稿删除、repair commit 和 GC 携带打开时读取的 `expectedCoordinationRevision + expectedDeletionGeneration`，并在同一 `readwrite` 事务内校验后推进 `coordinationRevision`。打开画布必须在同一个只读事务中取得同 scope 的 marker、drafts 和 epoch 一致快照；过期 repair 必须重新解析。GC 删除前重新验证草稿仍过期且未被 marker 引用。“确认删除”只表示用户已经确认删除 Canvas 资源本身：本地 Canvas 可直接执行；云端 Canvas 只有在本客户端发起的 DELETE 收到匹配 Canvas ID 的正向 `deletionReceipt` 后才能执行。DELETE 首次成功和同一活动成员的幂等重放都返回该回执；普通 GET/LIST 的 `404 canvas_not_found`、Workspace 非 active、成员移除、网络失败或任何不确定响应都不得被解释为删除证明，只保留草稿并显示受控不可用。确认后在单个事务中递增 `deletionGeneration`、写 tombstone 并删除该 scope 的 drafts/markers；tombstone 长期保留，首版不恢复同一 Canvas ID，若要找回内容必须创建新的 Canvas ID/scope。接受服务端版本、解决冲突、关闭 Session、普通草稿清理和 GC 都属于外部/自有草稿删除，只推进 `coordinationRevision`，绝不能写 tombstone 或推进 `deletionGeneration`。
...
租户隔离测试由 schema owner 只负责建库/迁移，并单独断言该 owner 没有 `BYPASSRLS`；实际请求使用生产等价的 `app_api`、`app_worker` 和 `app_maintenance` 登录角色。测试自身断言三者均非 owner、均无 `BYPASSRLS`，并证明缺少上下文、缺少显式 Workspace 条件、错误 join、伪造路径 Workspace、API 伪设无成员 Workspace、跨租户子资源 ID、伪造/跨事务复用 admin operation、邀请接受和池连接复用均默认拒绝；真实平台管理员事务只能访问 operation 绑定的目标 Workspace，并按动作矩阵处理 active/suspended/deactivated，跨 Workspace 或错误动作仍拒绝。状态机测试覆盖 team owner 专用 policy 的 active→deactivated、personal owner 自助停用被拒绝、管理员 active→suspended→deactivated、suspended/deactivated→active，并证明 deactivated 下 `wallet_adjust` 失败、restore 后才恢复 active 动作集合。平台目标 operation 只能按 read/write purpose 读取或修改 Model/Route；普通 app_api 读取 Route、读取 disabled Model、Workspace 目标 operation 和错误 purpose 必须失败，管理响应必须断言不含 `secret_ref` 或真实 Secret。Maintenance 只能读取清单中的扫描列，任何租户表写入或未授权列读取均失败，候选交给 Worker 后必须重新验证。不得以 schema owner 或超级用户运行后宣称 RLS 生效。
...
自动化测试还必须穿过真实的 Session/Manager adapter，而不只测存储原语：取消的 prepare 不写入、普通 open 原子提交 repairs、server-copy 使用 `repairs: []`、过期 repair 重新解析、双标签冲突保留两份入口、未知 marker 所有权时跳过 GC、服务端删除失败/不确定/普通 404/Workspace suspended 时不写 tombstone且恢复后草稿仍可用、DELETE 正向回执后写 tombstone且旧会话不能复活、接受服务端版本清除冲突草稿后新 Session 仍能在原 scope 正常写入、forced dispose abort 自有事务。当前 server-copy 漏传 `repairs` 的路径先作为红色回归测试，clean/server 结果改由统一构造函数产生。
