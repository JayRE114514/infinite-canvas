...
- 架构正确性、数据完整性和长期可维护性优先于上线期限；期限只能用于缩减功能范围，不能作为采用临时架构或跳过关键验证的理由。
- 新增或推翻核心架构前，必须先形成书面方案，明确公共边界、基础不变量、验证方式和人工验收范围，并通过一次独立对抗性方案审查；已批准架构下的实现切片不重复做独立审查，统一在可运行里程碑集成后验收。未经验证的原型只能留在隔离实验中，不能继续堆叠为正式实现。
- 写代码保持最少行数，能简单实现就不要引入复杂抽象。
- 标准格式、协议、解析、压缩、加密、日期等通用能力优先使用成熟稳定的库，不要手写底层实现，除非用户明确要求或项目已有实现必须沿用。
- 只实现当前明确需要的功能及维持现有权限、数据完整性、错误处理所需的基础不变量；不要为未提出的容灾、安全框架、扩展性或兼容场景增加设计。
- 项目尚未上线，不需要兼容旧数据；本地存储结构调整时直接按新设计修改，不写旧字段兼容或数据迁移兜底，除非用户明确要求。
- 单个轻量切片完成后，只运行与该切片直接相关的最小功能测试；不默认检查语法、运行全量测试或执行构建。
- 不要改无关文件，不要顺手重构。
...
- Matt Pocock 正式技能包采用用户级全局安装，项目中不复制第三方技能源码；本项目使用 `.scratch/` 本地 tracker 和单一领域上下文，配置见 `docs/agents/`，完整维护方式和调用示例见 `docs/matt-pocock-skills.md`。
- 允许自动触发的技能只有 `engineering-core`、`grilling`、`domain-modeling`、`codebase-design`、`diagnosing-bugs`、`writing-for-agents`、`tdd`、`code-review`；其中 `tdd` 和 `code-review` 只能在下方规定的里程碑条件满足时自动触发，所有技能都不得扩大任务权限。
- 其余 Matt 技能只能由用户通过 `$skill-name` 显式调用。普通功能和缺陷修改由主 Agent 直接处理，不需要机械执行完整技能链路。
- `grilling` 自动触发时，Agent 必须总结已确认决定、未决项和实施边界，并等待用户明确确认后才能实施；`grill-with-docs` 和 `grill-me` 仍只能由用户显式调用。
- 所有技能都必须服从本文件：测试范围、实现/验收分工与最多两轮验收规则不得被技能覆盖，提交、合并、关闭 PR 等外部状态变更仍需按本文件取得授权。

## 开发与验收流程

- 明确、局部、单一写集的需求直接开发；关键需求或设计不清时先使用 `grilling`，用户确认共同理解后再进入实施。
- 普通切片只实现一个最小完整行为，完成后运行一个与该行为直接相关的最小功能测试，不为单切片机械建立完整红绿循环、全量回归或独立验收。积分、授权、确认删除、数据丢失和并发 CAS 等不可逆高风险不变量允许先建立一个最小失败信号，但仍不得借此扩大测试范围。
- 多个切片共同形成一个完整、可运行的用户能力时构成里程碑，不按固定任务数量划分。此时自动使用 `tdd` 补齐集成后的行为缺口：运行受影响功能的完整测试集和必要集成测试，先暴露失败再逐项修复至通过；不追溯要求此前每个切片都重新按 test-first 实现。除非测试依赖构建或用户明确要求，否则不执行全仓库构建。
- 每个里程碑完成测试后自动使用 `code-review`：主 Agent 固定 diff 和测试证据，只由一个 Opus 或 Kimi 子代理综合验收需求价值、实现质量、基础不变量和故障路径。若有阻断问题，修复后只复验一次。
- 单切片只维护本地工作记录；里程碑完成独立验收后统一检查并更新 todo、pending-test 和 CHANGELOG，用户确认测试通过后再更新正式功能说明。
...
- 项目后端接口统一放在 `web/src/services/api/`。平台托管的身份、Workspace、Canvas、Asset、积分和 AI Task 必须通过项目后端访问。只有明确标记为本地或过渡模式的第三方资源才允许浏览器直连，不得再假设项目不存在后端。
...
- 每个可运行里程碑完成独立验收后，都要根据实际变更检查并更新 `docs/content/docs/progress/todo.mdx` 和 `docs/content/docs/progress/pending-test.mdx`；单切片不重复更新正式进度文档。功能或待办没有变化时，也要确认无需修改。
...
- 子代理分工必须区分“实现”和“验收”：`gpt-5.6-sol` 可承担实现，但不得承担验收；测试验收与对抗性审查仅由 Opus 或 Kimi 子代理执行。不得实现需求之外的容灾、安全框架或扩展机制，但必须维持现有权限、数据完整性和错误处理等基础不变量。实现代理的自测只能作为开发证据，不能替代独立验收。
- 任一任务的独立审查与验收测试最多两轮。第一轮从第一性原理集中验证正确性、安全边界和故障路径；如有阻断问题，修复后仅再进行一轮复验，避免无限细化和重复审查。
- 平台管理员以非成员身份读取或变更租户数据时，必须通过固定 `search_path`、从 xid 绑定操作推导 actor/target/purpose 的窄口 `SECURITY DEFINER` 数据库函数，并在同一事务内强制写入恰好一条不可变审计；不得用直接 RLS 放行或共享列权限把审计留给 TypeScript 调用顺序约定。
- 登录后的 Canvas 快照与 revision 以 PostgreSQL 服务端版本为业务权威；原生 IndexedDB 只保存未同步恢复草稿和 `localUi`。媒体字节和“我的素材”在 Asset 切换前仍主要保存在浏览器，必须与 Canvas 权威数据分开描述；不得把目标架构或本地恢复数据库误写成已上线能力或画布权威存储。
...
- 等待 Opus 等长时子代理审查时，单次 `wait_agent` 使用至少 30 分钟的长等待；工具分段 yield 时只续同一个等待 cell，不要每 5 分钟重新调用或切换模型。
