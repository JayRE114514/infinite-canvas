...
- 架构正确性、数据安全和长期可维护性优先于上线期限；期限只能用于缩减功能范围，不能作为降低基础设施质量、跳过验证或采用临时架构的理由。
- 新增或推翻核心架构前，必须先形成书面规格，经过独立对抗性审查，并定义可执行的自动化测试与人工验收矩阵；没有完成这些验证设计的架构不得进入实现。
- 不得把未经验证的方案作为正式实现尝试；原型或推测只能留在独立实验中，明确标记为不可合并，验证失败时先回到架构设计，不继续堆叠补丁。
...
- 子代理分工必须区分“实现”和“验收”：`gpt-5.6-sol` 可承担实现，但不得承担验收；测试验收与对抗性审查仅由 Opus 或 Kimi 子代理执行。实现代理的自测只能作为开发证据，不能替代独立验收。
